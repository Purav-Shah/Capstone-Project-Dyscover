...
export default function SpeakingPhonemePage() {
  ...
  async function finish(currentScore) {
    ...
    try {
      localStorage.setItem("phoneme_3_5", JSON.stringify(payload))
      ...
      <div className="relative z-10 flex items-center justify-center min-h-[calc(100vh-120px)] px-6">
        <div className="w-full max-w-3xl">
          ...
          >
            ...
            <div
              className={`text-4xl tracking-[1.25em] font-bold mb-6 ${isDarkMode ? "text-slate-100" : "text-slate-800"}`}
            >
              {item.letters.join(" ")}
            </div>
            <div className="flex flex-wrap items-center justify-center gap-4">
              {shuffle(item.options).map((opt, i) => (
                <button
                  key={i}
                  onClick={() => choose(opt)}
                  className="px-5 py-3 rounded-full bg-indigo-600 text-white hover:bg-indigo-700 active:scale-95 transition-all"
                >
                  {opt}
                </button>
              ))}
